...
<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    ...
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
        <a href="{{ url('/sales/sales_consignments/create') }}" class="button text-white bg-theme-1 shadow-md mr-2" id="add-button">Tambah {{$title}}</a>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function drawDatatable() {
        $("#main-table").DataTable({
            ...
            "serverSide": true,
            "ajax":{
                "url": API_URL+"/api/sales_consignment_datatables",
    ...
    $(document).on('click', 'button#delete-data', function( e ) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
                type: 'DELETE',
                url: API_URL+"/api/sales_consignments/"+id,
                headers: {
