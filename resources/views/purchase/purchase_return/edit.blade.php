...
<script type="text/javascript">
	...
	function getPurchaseReturn() {
        $.ajax({
            ...
            success: function(res, textStatus, jqXHR){
				...
				res.data.purchase_return_details.forEach((item, key) => {
					index = key
					$('#details-table tbody').append(setHtmlItemEdit(item))
					initSelect2()
					getRawMaterials(item.raw_material_id, key)
				})
    ...
    async function getRawMaterials(raw_material_id = null, key = null) {
        await $.ajax({
            ...
            success: function(res, textStatus, jqXHR){
				...
				res.data.forEach((item, index) => {
					if (raw_material_id) {
                    	opt += '<option value="'+item.id+'" '+(raw_material_id === item.id ? 'selected' : '')+'>'+item.name+'</option>'
					} else {
                    	opt += '<option value="'+item.id+'">'+item.name+'</option>'
					}
				})
                $('#input-raw-material-id-'+(key != null ? key : index)).html(opt)
            },
            error: function(jqXHR, textStatus, errorThrown){
	...
	function setHtmlItemEdit(item) {
		...
		html += '<tr class="item-'+index+'">'
		html += '	<td>'
		html += '		<select value="'+(item ? item.raw_material_id : '')+'" name="raw_material_id[]" id="input-raw-material-id-'+index+'" class="input w-full border mt-2 flex-1"></select>'
		html += '	</td>'
		html += '	<td>'
