...
<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    ...
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
        <a href="{{ url('/production/create') }}" class="button text-white bg-theme-1 shadow-md mr-2" id="add-button">Tambah {{$title}}</a>
    </div>
</div>
    ...
    <table class="table table-report table-report--bordered display datatable w-full" id="main-table">
        <thead>
            <tr>
                ...
                <th class="border-b-2 text-center whitespace-no-wrap">Total HPP</th>
                <th class="border-b-2 text-center whitespace-no-wrap">Overhead (%)</th>
                <th class="border-b-2 text-center whitespace-no-wrap">Overhead (Rp)</th>
                <th class="border-b-2 text-center whitespace-no-wrap">Margin (%)</th>
                <th class="border-b-2 text-center whitespace-no-wrap">Margin (Rp)</th>
...
<script type="text/javascript">
    ...
    $(document).on("click", "button#edit-data",function(e) {
      e.preventDefault();
      let id = $(this).data('id')
	  window.location.replace(`/sales/sales/edit/${id}`)
    });
    ...
    function drawDatatable() {
        $("#main-table").DataTable({
            ...
            "serverSide": true,
            "ajax":{
                "url": API_URL+"/api/recipe_datatables",
